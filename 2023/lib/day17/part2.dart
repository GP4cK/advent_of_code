...
List<({(int, int) pos, int cost, Axis from})> getNeighbors(
  ...
  switch (current.from) {
    case Axis.y:
      var cost = grid[y].sublist(x + 1, min(x + 4, grid.first.length)).sum;
      final maxX = min(x + 10, grid.first.length - 1);
      for (var i = x + 4; i <= maxX; i++) {
        cost += grid[y][i];
        neighbors.add((
          pos: (i, y),
          cost: cost,
      ...
      cost = grid[y].sublist(max(0, x - 3), x).sum;
      final minX = max(0, x - 10);
      for (var i = x - 4; i >= minX; i--) {
        cost += grid[y][i];
        neighbors.add((
          pos: (i, y),
          cost: cost,
    ...
    case Axis.x:
      var cost =
          grid.sublist(y + 1, min(y + 4, grid.length)).map((row) => row[x]).sum;
      final maxY = min(y + 10, grid.length - 1);
      for (var i = y + 4; i <= maxY; i++) {
        cost += grid[i][x];
        neighbors.add((
          pos: (x, i),
          cost: cost,
      ...
      cost = grid.sublist(max(0, y - 3), y).map((row) => row[x]).sum;
      final minY = max(0, y - 10);
      for (var i = y - 4; i >= minY; i--) {
        cost += grid[i][x];
        neighbors.add((
          pos: (x, i),
          cost: cost,
